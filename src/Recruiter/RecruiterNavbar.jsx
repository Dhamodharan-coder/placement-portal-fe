...
const {recruiternavimage} = useContext(Pipeline);
    const navigate = useNavigate();
    const login = localStorage.getItem("token2");
    function profilefun(){
    if(profiletoggle){
  ...
  <div class="mx-auto max-w-7xl px-2 sm:px-6 lg:px-8">
    <div class="relative flex h-16 items-center justify-between">
      ...
      <div class="flex-2">
          ...
          <h1 className='text-3xl font-bold text-stone-200 mx-10'>Dhru Recruiter Placement Portal</h1>
          </div>
        </div>
        ...
        <div class="relative ml-3">
          <div>
            <button type="button" class="relative flex rounded-full bg-gray-800 text-sm focus:outline-none focus:ring-2 focus:ring-white focus:ring-offset-2 focus:ring-offset-gray-800" id="user-menu-button" aria-expanded="false" aria-haspopup="true" onClick={profilefun}>
              ...
              <img class="h-12 w-12 rounded-full" src={`${recruiternavimage ? recruiternavimage : "https://img.icons8.com/3d-fluency/94/FA5252/person-male--v5.png"}`} alt="" />
            </button>
          </div>
